...
The repository is at the foundation stage. The current runnable components are `@mensura/shared-types`, the minimal Python 3.12 Mensura Core HTTP API, and the Tauri/React Mensura Studio shell. Studio can select a workspace, inspect local Git state, build a bounded Vault inventory and immutable context pack, run configured Guard checks, create a ready task and context-bound queued run, then manually execute through the credential-free deterministic provider or one optional locally configured OpenAI BYOK adapter. Execution records explicit `queued -> running -> succeeded | failed` state, provider kind/model, `review.v2` prompt identity, timestamps, and bounded schema-validated output. A successful run can materialize one separate write-isolated change proposal with immutable lineage, bounded file suggestions, and explicit approve/reject review state. Approval does not apply, stage, commit, or otherwise write changes; instead, an approved proposal can be verified in a temporary isolated Git worktree where Core materializes the suggestion, runs the configured Guard checks, and returns a separate verification artifact with sandbox, per-file, Guard, and safe-diff metadata before removing the sandbox. Verification never writes the live branch or working tree. A separately gated, explicit apply-to-live step then consumes an approved proposal plus a passing verification to write the exact digest-checked content to the live working tree—refusing before any write when a live file has drifted, staging atomic writes, re-running Guard against the result, and recording a separate auditable application artifact with per-file digests and undo metadata—while still never staging, committing, or pushing, and without any provider access. Apply and the digest-guarded undo share a single per-workspace write reservation, so two live-tree writers can never interleave on the same workspace whether they arrive as synchronous requests or background jobs; a refused writer returns a structured `409` and writes nothing. Repository and Vault access remain read-only, and provider execution receives only the persisted task plus immutable pack—not the live workspace path. OpenAI keys are write-only through Studio and stored by Core in the OS credential backend; only the model ID is saved in local user config. Core uses SQLite-backed durable persistence for all domain entities and background jobs; apart from the explicit apply-to-live write it does not modify the repository, and it never commits, stages, pushes, orchestrates agents, or loses history across restarts. At startup the durable app also performs conservative local hygiene—sweeping verification sandboxes orphaned by a previous crash and pruning old backups and terminal jobs under an explicit retention policy—without ever deleting core domain history. Separately, a workspace can be indexed into a durable Vault of chunked, embedded memory items with source/path/language/digest metadata; that index is searchable by embedding similarity through a pluggable local `Embedder` interface that uses a real local neural model (a local Ollama daemon, e.g. `nomic-embed-text`) when one is available and honestly falls back to an offline dependency-free lexical-vector model otherwise—always reporting which mode produced a result and recording the backend on the index. Lexical (sparse) indexes additionally persist a sparse inverted index alongside the chunks so search retrieves a small candidate set from the query's own buckets and reranks exactly that set with the same cosine + substring scorer—identical results to the former full scan but sub-linear as the index grows, with an explicit bounded fall-back to the linear scan for dense/legacy indexes, over-broad queries, or a corrupt acceleration index. The index can also produce a heuristic architecture summary of modules, technologies, and entry points; all read-only over the workspace and derived only from the persisted index, with no cloud embedding service.
...
- `services/core`: versioned FastAPI contracts, RFC 9457 errors, read-only Git/Vault adapters, a durable Vault index with chunked memory items, local semantic search (real Ollama embeddings when available, honest offline lexical-vector fallback otherwise) with a sub-linear sparse inverted-index candidate path for lexical indexes (exact-equivalent to the linear scan, with a bounded linear fall-back), and heuristic architecture summaries, immutable context-pack assembly, bounded Guard runner, deterministic plus optional OpenAI BYOK execution adapters, write-isolated proposal review, temporary Git-worktree verification sandboxes, explicit digest-checked apply-to-live with atomic writes and audit artifacts, bounded undo execution, a unified per-workspace live-tree write reservation shared by apply/undo and the worker, safe SQLite backup/restore, a durable SQLite-backed background job queue with an in-process worker and restart recovery, live SSE status updates, a startup maintenance pass (orphaned verification-sandbox sweep plus conservative backup/terminal-job retention), local credential/config boundaries, durable SQLite persistence, and both unit and HTTP-level durable-SQL integration tests.
