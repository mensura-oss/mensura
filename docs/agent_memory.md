...
- Current implementation status: the repository has a runnable pnpm workspace, tested shared contracts, a verified FastAPI Core service, and a Tauri/React Studio flow for workspace selection -> Vault inventory -> immutable context pack -> ready task -> context-bound queued run -> explicit deterministic or optional OpenAI BYOK execution -> separately persisted bounded change proposal -> approve/reject review -> optional isolated sandbox verification of approved proposals. Verification materializes the proposal only inside a temporary detached Git worktree, runs the configured Guard checks there, persists a separate artifact with sandbox/per-file/Guard/safe-diff metadata, and removes the sandbox; the live branch and working tree are never written. OpenAI keys use the OS credential backend, current `review.v2` output is code-bounded, `review.v1` remains versioned, and deterministic remains credential-free. Repository/Git inspection and Vault remain read-only. Durable resource persistence, final apply-to-live, additional providers, and orchestration remain unimplemented.
