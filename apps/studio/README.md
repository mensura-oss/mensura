...
The app does not start Core itself. Execution, proposal review, verification, and apply-to-live are manual, synchronous, and bounded; only apply-to-live writes the workspace, and it never invokes Git. OpenAI is the only optional real adapter and deterministic remains first-class. Verification, apply, undo, and backup can additionally run as durable queued jobs drained by an in-process Core worker, but Git writes (stage/commit/push/checkout/reset), additional vendors/models UX, arbitrary prompt editing, credential read-back, automatic job retries, job cancellation, external brokers/distributed workers, WebSockets, bidirectional events, guaranteed durable event replay, orchestration, Monaco, terminals, full repository tree/navigation, file editing, semantic search/embeddings, full diff/patch editing, task/run lists, Kanban, the full Guard policy engine/history/config editor, Hub, plugins, and authentication are intentionally deferred.
