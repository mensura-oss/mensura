...
- Vault memory panel that indexes the workspace into searchable chunks, shows indexed/not-indexed status with last-indexed time and code/doc/config/chunk counts, runs a ranked lexical-vector search with a `sourceType` filter, opens the matched chunk (with its line range) in a detail pane, and generates a basic architecture summary (modules, technologies, entry points)
