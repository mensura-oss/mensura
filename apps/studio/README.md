...
The app does not start Core itself. Execution, proposal review, verification, and apply-to-live are manual, synchronous, and bounded; only apply-to-live writes the workspace, and it never invokes Git. OpenAI is the only optional real adapter and deterministic remains first-class. Undo execution/rollback, Git writes (stage/commit/push/checkout/reset), additional vendors/models UX, arbitrary prompt editing, credential read-back, retry/cancellation, a worker/broker, streaming/SSE, orchestration, Monaco, terminals, full repository tree/navigation, file editing, semantic search/embeddings, full diff/patch editing, task/run lists, Kanban, durable Vault/context-pack/run/proposal/verification/application history or watchers, the full Guard policy engine/history/config editor, Hub, plugins, and authentication are intentionally deferred.
