...
- Vault memory panel that indexes the workspace into searchable chunks, shows indexed/not-indexed status with last-indexed time and code/doc/config/chunk counts, runs a ranked lexical-vector search with a `sourceType` filter, opens each hit in a line-numbered read-only file view that scrolls to and highlights the matched line range, and generates a basic architecture summary (modules, technologies, entry points)
